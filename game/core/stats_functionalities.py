...
def format_datetime(date_time: datetime.datetime, type_="date"):
    """
    ...
    `type_`: - The format of the datetime object to be converted to
    """
...
def get_fastest_time(uid: str, diff_id: int):
    """
    Gets the user's fastest completion time and the corresponding date for the specified difficulty
    ...
    if ft_record is None:
        return None

    else:
        timer_value = get_timer_value(fastest_timedelta)

        return f"{timer_value} ", f"({format_datetime(ft_record.end_time, type_='date')})"
