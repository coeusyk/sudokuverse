...
from game.core.constants import USER_IDENTIFIER, NUM_OF_DIFFICULTIES, DIFFICULTY_DICT, DIFF_CHOSEN, SUCCESS_RESP, DIFF_RESP, SIMPLE, MEDIUM, COMPLEX
from game.core.stats_functionalities import format_datetime, get_fastest_time, get_games_per_diff, get_timer_value


home_blueprint = Blueprint("home_blueprint", __name__, template_folder="templates", static_folder="static", static_url_path="/ui/home")
...
def home_window():
    if USER_IDENTIFIER in request.cookies:
        if request.cookies[USER_IDENTIFIER] != "logged-out":
            ...
            games_finished = GameStats.query.filter_by(uid=uid, game_result=1).order_by(GameStats.start_time.desc()).all()
    ...
    if request.method == "POST":
        ...
        if request.headers["Content-Type"] == "application/x-www-form-urlencoded":
            ...
            if "log-out" in content:
                SUCCESS_RESP.set_cookie(USER_IDENTIFIER, "logged-out")  # Updating the user cookie to "logged-out"
    ...
    if request.method == "GET":
        ...
        if request.headers["Accept"] == "application/json":
            completed_games_info = {"entry-id": [], "difficulty": [], "hints-used": [], "time-taken": [], "date-time": []}

            # Sending the latest (upto) five games to the client (JS):
            for cg in games_finished[:5]:
                cg: GameStats

                diff: str = list(filter(lambda key: DIFFICULTY_DICT[key] == cg.game_type, DIFFICULTY_DICT))[0]  # Returns a list which contains all keys having the value of "game_type"
                ...
                completed_games_info["date-time"] += [[format_datetime(cg.end_time, type="date"), format_datetime(cg.end_time, type="time")]]
            
            cg_info_response = Response(json.dumps(completed_games_info), status=200)
